...
export const ChatWidget = () => {
  ...
  const handleSendMessage = (text) => {
    ...
    if (matchingQuery) {
      ...
      if (matchingQuery.text === 'About SK Smart / MD') shouldRedirectAbout = true;
    } else if (
      queryText.includes('sk') || queryText.includes('company') || 
      queryText.includes('about') || queryText.includes('details') || 
      queryText.includes('director') || queryText.includes('prakash')
    ) {
    ...
    setTimeout(() => {
      ...
      if (shouldRedirectAbout) {
        setTimeout(() => { navigate('/about'); setIsOpen(false); }, 2200);
      }
    }, 1200);
  };
  ...
  return (
    <>
      ...
      <div className={`fixed bottom-6 right-6 lg:bottom-8 lg:right-8 z-[9999] font-sans transition-all duration-300 ${isMinimized && !isOpen ? 'translate-x-[200%] opacity-0 pointer-events-none' : 'translate-x-0 opacity-100'}`}>
        <AnimatePresence>
          {isOpen && (
            <motion.div
            ...
            >
              ...
              <div className="bg-gradient-to-r from-navy-900 to-navy-950 dark:from-navy-950 dark:to-navy-900 p-4 border-b border-slate-200/40 dark:border-white/5 flex items-center justify-between text-white">
                ...
                <button onClick={() => setIsOpen(false)} className="p-2 text-slate-400 hover:text-white rounded-xl hover:bg-white/10 transition-all cursor-pointer">
                  <FaTimes />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 dark:bg-navy-900/10 text-xs">
                ...
                {isTyping && (
                  <div className="flex items-start space-x-2.5">
                    <div className="w-7 h-7 rounded-full bg-slate-200 dark:bg-navy-800 flex items-center justify-center text-[10px] text-navy-950 dark:text-white"><FaRobot /></div>
              ...
              {messages.length === 1 && !isTyping && (
                <div className="px-4 py-2 bg-slate-50/50 dark:bg-navy-900/20 border-t border-slate-200/20 dark:border-white/5 text-left">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {quickQueries.map((q) => (
                      <button key={q.text} onClick={() => handleSendMessage(q.text)}
              ...
              >
                <input type="text" placeholder="Ask your query here..." value={inputValue} onChange={(e) => setInputValue(e.target.value)}
                ...
                />
                <button type="submit" className="p-2.5 bg-gold-500 hover:bg-gold-600 text-white rounded-xl transition-all cursor-pointer transform active:scale-95">
